...
func (f *AckFrame) Append(b []byte, version protocol.Version) ([]byte, error) {
	if len(f.ReceiveTimestamps) > 0 {
		return f.appendReceiveTimestamps(b, version)
	}
...
func (f *AckFrame) Length(version protocol.Version) protocol.ByteCount {
	if len(f.ReceiveTimestamps) > 0 {
		return f.receiveTimestampsLength(version)
	}
...
func (f *AckFrame) Truncate(maxSize protocol.ByteCount, version protocol.Version) {
	if len(f.ReceiveTimestamps) > 0 {
		// Preserve ACK correctness if the packet budget cannot carry all feedback.
		// Missing timestamps are explicitly detected by the SBD sender.
		for len(f.ReceiveTimestamps) > 0 && f.Length(version) > maxSize {
			f.ReceiveTimestamps = f.ReceiveTimestamps[:len(f.ReceiveTimestamps)-1]
		}
		if len(f.ReceiveTimestamps) > 0 {
			return
		}
