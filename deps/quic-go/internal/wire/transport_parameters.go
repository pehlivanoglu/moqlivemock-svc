...
type TransportParameters struct {
	ReceiveTimestampsEnabled       uint64
	MaxReceiveTimestamps           uint64
...
func (p *TransportParameters) unmarshal(b []byte, sentBy protocol.Perspective, fromSessionTicket bool) error {
	...
	for len(b) > 0 {
		...
		case maxIdleTimeoutParameterID,
			...
			minAckDelayParameterID, 0xff0a001, 0xff0a002, 0xff0a003:
...
func (p *TransportParameters) readNumericTransportParameter(b []byte, paramID transportParameterID, expectedLen int) error {
	...
	case 0xff0a001:
		if val > 1 {
			return errors.New("invalid receive timestamps enable value")
		}
		p.ReceiveTimestampsEnabled = val
	case 0xff0a002:
		p.MaxReceiveTimestamps = val
	case 0xff0a003:
		if val > 20 {
			return errors.New("receive timestamp exponent exceeds 20")
		}
		p.ReceiveTimestampsExponent = val
...
func (p *TransportParameters) Marshal(pers protocol.Perspective) []byte {
	...
	if pers == protocol.PerspectiveClient {
		b = p.marshalVarintParam(b, 0xff0a001, 1)
		b = p.marshalVarintParam(b, 0xff0a002, 0)
		b = p.marshalVarintParam(b, 0xff0a003, 0)
	}
